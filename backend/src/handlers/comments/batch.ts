...
const logger = createLogger('comments-batch');
...
export async function handler(event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> {
  try {
    ...
    const commentPreviews = await query<CommentQueryResult>(
      `SELECT c.article_id, c.comment_id, c.user_id, u.username, c.content
       FROM comments c
    ...
    logger.error('Failed to fetch batch comments', { error: err });
